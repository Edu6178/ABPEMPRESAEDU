...
<head>
    ...
    <title>Editar Ciudad - Distribuidora ABC</title>
                ...
                <form action="{{ route('ciudades.update', $ciudad->id_ciudad) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="mb-4">
                            ...
                            <input type="text" name="nombre_ciudad" id="nombre_ciudad" value="{{ $ciudad->nombre_ciudad }}" class="w-full p-3 mt-1 border border-gray-300 rounded-lg" required>
                        </div>

                        <div class="mb-4">
                            <label for="estado" class="block text-sm font-semibold text-gray-700">Estado</label>
                            <input type="text" name="estado" id="estado" value="{{ $ciudad->estado }}" class="w-full p-3 mt-1 border border-gray-300 rounded-lg" required>
                        </div>

                        <div class="mb-4">
                            <label for="codigo_postal" class="block text-sm font-semibold text-gray-700">Código Postal</label>
                            <input type="text" name="codigo_postal" id="codigo_postal" value="{{ $ciudad->codigo_postal }}" class="w-full p-3 mt-1 border border-gray-300 rounded-lg" required>
                        </div>
                    </div>

                    <div class="mt-4">
                        <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-lg">Actualizar Ciudad</button>
                    </div>
                </form>
            </div>
        </div>
