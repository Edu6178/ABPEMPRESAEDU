...
<head>
    ...
    <title>Editar Producto - Distribuidora ABC</title>
                ...
                <form action="{{ route('productos.update', $producto) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="mb-4">
                            ...
                            <input type="text" name="nombre" id="nombre" value="{{ $producto->nombre }}" class="w-full p-3 mt-1 border border-gray-300 rounded-lg" required>
                            @error('nombre')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="descripcion" id="descripcion" class="w-full p-3 mt-1 border border-gray-300 rounded-lg" required>{{ $producto->descripcion }}</textarea>
                            @error('descripcion')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" name="precio" id="precio" value="{{ $producto->precio }}" class="w-full p-3 mt-1 border border-gray-300 rounded-lg" required>
                            @error('precio')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @if ($producto->imagen)
                                <img src="{{ asset('storage/'.$producto->imagen) }}" alt="Imagen del producto" class="mt-2 w-32 h-32 object-cover">
                            @endif
                            @error('imagen')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="mt-4">
                        <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-lg">Actualizar Producto</button>
                    </div>
                </form>
            </div>
        </div>
